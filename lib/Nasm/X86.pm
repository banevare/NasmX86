...
my @extern;
 # External symbols imports for linking with C libraries
my @link;
 # Specify libraries which to link against in the final assembly stage
my @text;                                                                       # Code
...
=item B<CallWithArgsX([$function_struct | 'label_string'], @arguments)>
  ...
  Call a function declared with 'Subroutine' or a function in the global namespace with linux x86_64 calling order convention for C-library compatible calls
...
END
  ...
  #NOTE: Could be different on other distributions. Required for linking against other libraries.
  # We leave it up to the user to specify which libraries to link via the Link subroutine.
  # The interpreter is required for the linked executables to work properly.
  # It uses a different one by default so we have to specify
  my $interpreter = scalar(@link) != 0 ? '-I /usr/lib64/ld-linux-x86-64.so.2' : ''; 
  my $cmd  = qq(nasm -f elf64 -g -l $l -o $o $c && ld @link $interpreter -o $e $o  && chmod 744 $e);# Assemble
  my $o1 = 'zzzOut.txt';
  my $o2 = 'zzzErr.txt';
  my $out  = $k ? '' : "1>$o1";
  my $err  = $k ? '' : "2>$o2";
